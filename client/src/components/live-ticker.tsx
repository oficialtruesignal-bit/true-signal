...
export function LiveTicker({ signals }: { signals: Signal[] }) {
  return (
    <div className="w-full bg-card border-y border-white/5 overflow-hidden py-2 mb-6">
      <div className="flex animate-scroll whitespace-nowrap">
        {[...signals, ...signals].map((signal, i) => (
          <div key={`${signal.id}-${i}`} className="flex items-center gap-2 mx-6 text-sm">
            <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
            <span className="font-bold text-white">{signal.homeTeam} vs {signal.awayTeam}</span>
